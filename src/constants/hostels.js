const hostels = [
  {
    id: 1,
    name: "Campus View Hostel",
    location: "Kirinyaga University • 300m away",
    price: 7500,
    rating: 4.9,
    image:"https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=900",

    images:[
           "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=900",
            "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=900",
             "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=900",
              "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=900",
    ],
    amenities: ["WiFi", "Water", "Parking","Security","Electricity"],
     description: "Campus View Hostel offers spacious modern rooms, reliable WiFi, clean water throughout the week, 24-hour security and a quiet environment perfect for students.",

  },

  {
    id: 2,
    name: "Royal Heights",
    location: "Kirinyaga University • 500m away",
    price: 6800,
    rating: 4.7,
    image: "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=900",

    images:[
      "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=900",
       "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=900",
        "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=900",
         "https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?w=900",
    ],
    amenities: ["WiFi", "Security", "Parking","Security"],
    description:"View is fantastic,reliable Wifi,Friendly neighbourhood,fresh water from well,24-hr security.",
  },

  {
    id: 3,
    name: "Green Court",
    location: "Kirinyaga University • 1km away",
    price: 5500,
    rating: 4.5,
    image:"https://images.unsplash.com/photo-1494526585095-c41746248156?w=900",

    images:[
      "https://images.unsplash.com/photo-1494526585095-c41746248156?w=900",
       "https://images.unsplash.com/photo-1494526585095-c41746248156?w=900",
        "https://images.unsplash.com/photo-1494526585095-c41746248156?w=900",
         "https://images.unsplash.com/photo-1494526585095-c41746248156?w=900",
    ],
    amenities: ["Water", "WiFi","Electricity"],
    description:"Green environment full of trees,near Nkomongo market,Reliable wifi,clean water.",
  },
];

export default hostels;